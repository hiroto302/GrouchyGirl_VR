...
public class PlayerTalkScript : MonoBehaviour
{
    ...
    // // LateUpdateメソッド Updateメソッドと同じように毎フレーム呼ばれるが、Updateより後に呼ばれる
    // private void LateUpdate() {
    //     // 会話相手がいる場合はTalkIconの位置を会話相手の頭上に表示
    //     if (conversationPartner != null) {
    //         // Camera.main.GetComponent().WorldToScreenPointでメインカメラのCameraコンポーネントのWorldToScreenPointメソッドを使って引数で与えられたワールド位置をカメラのスクリーンの位置に変換
    //         // こうすることでTalkIconのサイズが変更せず会話相手が移動してもそのワールド位置をカメラのスクリーンの位置に変換し移動するようになる
    ...
    // 会話相手を設定
    // 会話相手が範囲に入ってきた時、実行されるメソッド(ConversationScopeScriptで実行,引数にはFirstManが代入されてくる)
    ...
    // 会話相手をリセット
    // 会話相手が設定されていなかったり、他の村人が会話相手として設定されている時に別の村人を引数で受け取っても会話相手はリセットしないようにした
    ...
    public void StartTalking() {
        var FirstManScript = conversationPartner.GetComponent<FirstManScript>();
        FirstManScript.SetState(FirstManScript.State.Talk, transform);
        this.allMessage = FirstManScript.GetConversation().GetConversationMessage();
    ...
    void EndTalking() {
        ...
        var FirstManScript = conversationPartner.GetComponent<FirstManScript>();
        FirstManScript.SetState(FirstManScript.State.Wait);
        // GetComponent<MoveController>().SetState(MoveController.State.Normal);
        // Input.ResetInputAxes();  //オキュラスクエスト対応の入力リセット方法
    }
}